...
import { Consulta8DataSource, Consulta8Item } from './consulta8-datasource';
...
export class Consulta8Component implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator) paginator: MatPaginator;
  displayedColumns: string[] = ['No', 'Inicial', 'Territorio'];
  dataSource: any;
  COUNTRIES: Consulta3[];
  // page = 1;
  // pageSize = 4;
  ...
  loadConsulta() {
    return this.restApi.getconsulta8_1().subscribe((data: {}) => {
      if (data instanceof Array)
      {
        ...
        this.COUNTRIES = [];
        data.forEach(dato => {
          this.COUNTRIES.push({id: numero, INICIAL: dato.INICIAL_PAIS, TERRITORIO: `${dato.AREA_kM2}  KM2`});
          numero++;
        });
